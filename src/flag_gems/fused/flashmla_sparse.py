...
def _prune_flash_mla_sparse_configs(configs, named_args, **kwargs):
    _ = kwargs
    hq = named_args["HQ"]
    # Avoid extra tuning work for shapes with an obvious head tile size.
    if hq <= 16:
        return [config for config in configs if config.kwargs["BH"] == 16]
    if hq >= 64:
        return [config for config in configs if config.kwargs["BH"] == 64]
    return configs
...
def _can_use_tle_flash_mla_sparse_fwd(
    ...
    # The TLE path has only been validated for FlashMLA's original head counts.
    return (
        d_v == 512
        and HKV == 1
        and DQK in (512, 576)
