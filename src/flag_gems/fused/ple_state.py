...
"""Stride-aware PLE state I/O for eager and graph execution.

Accelerators use the same kernels in both modes, avoiding materialization of
large transposed caches through index_select. CPU tensor paths provide
numerical references for focused tests.
"""
...
def _is_triton_device(tensor: torch.Tensor) -> bool:
    return tensor.device.type not in ("cpu", "meta")
