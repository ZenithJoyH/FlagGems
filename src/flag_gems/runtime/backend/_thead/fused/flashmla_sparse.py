...
"""T-Head Split-K specialization for small-head sparse MLA."""
...
def _flash_mla_sparse_splitk_stage1(
    ...
    # Entire trailing splits are common while context is growing. Skip their
    # Q/KV loads and dot products; every program still writes neutral partials
    # so the merge kernel remains branch-free.
    if split_start < valid_topk:
        ...
        for block_id in range(blocks_per_split):
            ...
            kv_ids = tl.load(
                index_base + positions, mask=position_mask, other=-1
            )
    ...
    stat_base = (
        (query_id * NUM_SPLITS + split_id) * HQ + offs_h
    )
    ...
    tl.store(
        partial_acc + acc_base + BDP + offs_d[None, :], acc1
    )
...
def _flash_mla_sparse_splitk_stage2(
    ...
    stat_offsets = (
        (query_id * NUM_SPLITS + offs_s) * HQ + head
    )
    ...
    acc_offsets = (
        (
            (query_id * NUM_SPLITS + offs_s[:, None]) * HQ
            + head
        )
        * DV
        + offs_d[None, :]
