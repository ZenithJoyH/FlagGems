...
def _inputs(heads: int, d_qk: int = 576):
    torch.manual_seed(20260907)
    q = (torch.randn(2, heads, d_qk, device="cuda") * 0.05).to(torch.bfloat16)
    kv = (torch.randn(256, 1, d_qk, device="cuda") * 0.05).to(torch.bfloat16)
...
@pytest.mark.parametrize("heads", [4, 17, 65, 256])
@pytest.mark.parametrize("d_qk", [512, 576])
def test_flash_mla_sparse_supports_nonstandard_head_counts(heads, d_qk):
    q, kv, indices, lengths, sinks = _inputs(heads, d_qk)
    scale = d_qk**-0.5
