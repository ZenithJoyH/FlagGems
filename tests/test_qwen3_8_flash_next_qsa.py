...
from tests.qwen3_8_flash_next_reference import (
...
def _load_qsa_ops():
    ...
    try:
        import flag_gems.fused as fused_ops

        return fused_ops
    except Exception as exc:  # target jobs must not hide import failures
        pytest.fail(f"FlagGems QSA import failed: {type(exc).__name__}: {exc}")
